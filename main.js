document.addEventListener('DOMContentLoaded', function () {
    ...
    const productsData = [{
        ...
        price: 45000,
        image: "https://avatars.mds.yandex.net/get-marketpic/8869508/pic4a5f6118ceb852a067743f72c4195ae0/orig",
        rating: 4.9,
        reviews: 28
    }];
...
// Функции для работы с корзиной
    ...
    function addToCart(product) {
        ...
        if (saveCart(cart)) {
            ...
            console.log('Корзина после добавления:', savedCart);
        }
    }
    ...
    function removeFromCart(productId) {
        ...
        if (saveCart(cart)) {
            console.log('Корзина после удаления:', getCart());
        }
    }
    ...
    function updateCartDisplay() {
        ...
        if (document.getElementById('cart-items-container')) {
            ...
            if (totalAmountEl) totalAmountEl.textContent = formatPrice(cartTotal);
            if (itemsCountEl) itemsCountEl.textContent = `${cartCount} товаров`;
            if (checkoutTotalEl) checkoutTotalEl.textContent = formatPrice(cartTotal);
...
// Получение текста категории
    function getCategoryText(category) {
        const categories = {
            'all': 'Все типы', 'cars': 'Для легковых', 'motorcycles': 'Для мотоциклов', 'tractors': 'Для тракторов'
        };
        ...
        document.addEventListener('click', function (e) {
            if (e.target.closest('.buy-btn')) {
                ...
                if (product) {
                    ...
                    console.error(`Товар с id=${productId} не найден в productsData`);
                }
            }
        });
    }
...
// Инициализация
    document.addEventListener('DOMContentLoaded', function () {
        // Инициализируем данные о товарах
        initProductsData();

        // Проверяем, что элементы корзины существуют
        ...
        productsGrid.addEventListener('click', function (e) {
            if (e.target.classList.contains('buy-btn')) {
                ...
                if (cartBtn) {
                    ...
                    let cartTotal = cartBtn.querySelector('.cart-total').innerText
                    cartTotal = cartTotal.replace('₽', '').replaceAll(" ", "").replace(" ", '')
                    cartTotal = String(Number(cartTotal) + Number(price)) + " ₽"
                    cartBtn.querySelector('.cart-total').innerText = cartTotal;
                    const cartCount = document.querySelector('.cart-count')
                    cartCount.innerText = String(1 + Number(cartCount.innerText))
                }
            }
        });
    }
    ...
    function applyFilters() {
        ...
        if (searchQuery) {
            ...
            filteredProducts = filteredProducts.filter(product => product.name.toLowerCase().includes(query) || product.description.toLowerCase().includes(query));
        }
    ...
    function getCategoryClass(category) {
        const classes = {
            'cars': 'badge-blue', 'motorcycles': 'badge-orange', 'tractors': 'badge-green'
        };
